...
function Product ({ data, type, className }) {
  ...
  const percentDiscount = data.discount
    ? Math.ceil(100 - (100 * data.discount) / data['price']['retailPrice'])
    : null
  function renderType () {
    if (data.new) {
  ...
  const addToWishlistHandle = e => {
    e.preventDefault()
    const wishlistItem = checkProductInWishList(wishlistData, data.productId)
            ...
            <div className='product__thumb__actions'>
              <div className='product-btn' data-tip data-for='cartIcon'>
                <Button
                  ...
                  className={`product__actions__item -round ${classNames({
                    'active -disable':
                      checkProductInCart(cartState, data.productId) ||
                      data.quantity < 1
                  })}
                  `}
                  action='#'
              ...
              <div className='product-btn' data-tip data-for='wlIcon'>
                <Button
                  ...
                  color='white'
                  className={`product__actions__item -round ${classNames({
                    active: checkProductInWishList(wishlistData, data.productId)
                  })}`}
                  action='#'
            ...
            <div className='product__content__footer'>
              <h5 className='product-price--main'>
                {data.discount
                  ...
                  : formatCurrency(data['price']['retailPrice'])}
              </h5>
              {data.discount && (
